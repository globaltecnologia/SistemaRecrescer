...
test("Usuários: incluir, pesquisar, abrir, alterar, excluir e validar obrigatórios", async ({ page }) => {
  ...
  await expect(page.getByLabel("Nome", { exact: true })).toHaveAttribute("required", "");
  await expect(page.getByLabel("Login", { exact: true })).toHaveAttribute("required", "");
  await expect(page.getByLabel("Senha", { exact: true })).toHaveAttribute("required", "");
  await page.getByLabel("Nome", { exact: true }).fill("Usuário E2E"); await page.getByLabel("Login", { exact: true }).fill(login); await page.getByLabel("Senha", { exact: true }).fill("SenhaE2E123!");
  ...
  await row.getByRole("button", { name: "Editar" }).click(); await expect(page.getByLabel("Login", { exact: true })).toHaveValue(login);
  await page.getByLabel("Login", { exact: true }).fill(changed); await page.getByRole("button", { name: "Salvar" }).click();
