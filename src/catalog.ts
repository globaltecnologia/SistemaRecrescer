...
export const entities: Partial<Record<PageKey, EntityDefinition>> = {
  ...
  enrollments: {
    ...
    fields: ({ students, classes, shifts }) => [
      ...
      text("student_name", "Nome do aluno"),
      { name: "birth_date", label: "Data de nascimento", type: "date" },
      text("nationality", "Nacionalidade"),
      text("birthplace", "Naturalidade"),
      text("previous_grade_course_shift", "Série/Curso/Turno anterior"),
      { name: "gender", label: "Sexo", type: "select", options: [{ label: "Masculino", value: "M" }, { label: "Feminino", value: "F" }, { label: "Outro", value: "O" }] },
      text("father_name", "Nome do pai"),
      text("father_phone", "Telefone do pai"),
      text("father_cpf", "CPF do pai"),
      text("mother_name", "Nome da mãe"),
      text("mother_phone", "Telefone da mãe"),
      text("mother_cpf", "CPF da mãe"),
      text("lives_with", "Mora com"),
      text("student_address", "Endereço do aluno"),
      text("student_phone", "Telefone do aluno"),
      text("guardian_name", "Nome do responsável"),
      text("guardian_relationship", "Grau de parentesco"),
      { name: "siblings_in_daycare", label: "Irmão na creche", type: "select", options: [{ label: "Sim", value: 1 }, { label: "Não", value: 0 }] },
      text("siblings_details", "Detalhes dos irmãos"),
      { name: "new_student", label: "Novo aluno", type: "select", options: [{ label: "Sim", value: 1 }, { label: "Não", value: 0 }] },
      ...
      text("contracted_hours", "Carga horária contratada"),
    ],
    columnLabels: {
      year: "Ano",
      student_name: "Nome do aluno",
      requested_class_id: "Turma / Turno",
    },
  },
  medical_records: {
